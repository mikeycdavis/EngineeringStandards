...
| `security.no-secrets-in-artifacts` reads the working tree, not the repository ([ADR 0008](artifacts/adr/0008-detectors-do-not-assert-repository-state-they-have-not-measured.md)) | It says *tracked* and means *present on disk*, so the seeded fakes in redaction tests are reported as committed credentials. Check `git ls-files` before you rotate anything — and do not write an exception for it, because the rule is wrong rather than inconvenient. **`scm.no-committed-env-files` no longer has this defect:** it consults the repository index through the ADR 0008 seam, so a gitignored `.env` is not a finding, and where the index cannot be read the rule reports `not-evaluated` rather than guessing |
| Structural rules pass on **gitignored** files ([ADR 0008](artifacts/adr/0008-detectors-do-not-assert-repository-state-they-have-not-measured.md)) | `planning.breakdown-directory`, `planning.one-file-per-section`, `architecture.project-manifest` and `documentation.architecture` establish their requirement by finding a path on disk. If the path is gitignored they pass anyway, and a fresh clone has none of it — the opposite polarity of the same defect, and the worse one, because nothing surfaces it. Check `git ls-files` covers your planning and documentation artifacts; one adopter's `.gitignore` carried `artifacts/` for the .NET SDK output convention and hid its entire plan |
| `security.no-secrets-in-artifacts` and `quality.unfinished-work` fire on fixtures ([ADR 0009](artifacts/adr/0009-detectors-distinguish-instances-of-a-subject-from-discussion-of-it.md)) | A test that seeds a credential to prove a scrubber removes it, and the sample a TODO scanner is tested against, are genuine instances whose intent is the only thing separating them from the real defect. That distinction may not be reliably automatable, so these are disclosed rather than guessed at. `quality.unfinished-work` also fires on prose *about* markers — a doc comment ending a clause with `TODO:` |
