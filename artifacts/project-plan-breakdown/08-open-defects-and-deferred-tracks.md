...
- **Status:** READY — **reopened 2026-08-24 by owner ruling.** This item held `COMPLETE` from
  ...
  **A seventh criterion is added 2026-08-24, rather than the sixth being reworded.** The
  ...
  | # | Acceptance criterion | Established by |
  |---|---|---|
  | 7 | A framework-caused loss of eligible project evidence makes the evidence surface incomplete | **Met 2026-08-25 on `fix/issue-7-evidence-surface-authority`, and it cost this repository nine of its own passes.** Every exclusion now records *who decided it* at the branch that decided — `project` for a repository ignore, `content` for a self-identifying marker tree, `framework` for a `SKIP_DIRS` name match — and a framework exclusion additionally records whether committed content sat behind it, asked of `git ls-files` through the existing `trackedMatching` seam rather than guessed from the name. `complete` and rule withdrawal are then derived from **one** list of loss modes instead of each enumerating its own: they had already drifted apart, `complete` counting five modes and withdrawal counting two, and neither counting exclusion. Falsifiers, four, each killing a distinct mutant: `a tool-decided exclusion over tracked content makes the surface incomplete` and `a content rule does not report passed over a tool-excluded tracked tree` both die if the exclusion term is removed; `a repository-authorized exclusion leaves the surface complete` and `a tool-decided exclusion that removes no committed content leaves the surface complete` both die if the term stops asking whether anything was actually lost, which is the naive fix criterion 7 was written to exclude; and the last of the four dies alone if the exclusion defeats completeness without withdrawing the rules, which is the state that made criterion 1 defeasible in the first place. `.git`, `dist` and `node_modules` hold nothing committed, so they cost the run nothing and the flag keeps its information: a criterion satisfied by making every repository incomplete would be satisfied by a flag nobody could read. **The consequence for this repository is the finding, not a side effect.** Its own audit excludes `test/fixtures` — 71 tracked, committed files — on the strength of the name, so its surface is now `complete: false` and `documentation.code-consistency`, `errors.no-swallowed-exceptions`, `planning.plan-code-consistency`, `security.no-secrets-in-artifacts`, `security.no-cert-bypass`, `security.no-sql-concat`, `verification.before-completion`, `quality.unfinished-work` and `quality.dead-code` move from `passed / evaluated` to `not-evaluated`. Those nine passes were resting on 71 unsearched tracked files, one of which carries deliberate SQL-concatenation bait, so `security.no-sql-concat: passed` was never established. `status` and `score` are unchanged — the four failures remain the four recorded rejections — and the self-audit test is pinned to exactly one loss mode over exactly one named tree rather than relaxed, so any further unsearched content fails it as the old assertion would have. **Why this does not close the item.** Whether `fixtures` and `vendor` are names a tool may act on at all is the separate question, and restoring this repository to a complete surface is a change to what it excludes rather than to what the audit reports. |
