...
Each row was re-verified against the implementation during reconciliation and holds, with one
detail worth adding: writing the attestation anyway does not leave the rule `skipped`. It produces
`disposition: invalid-attestation`, `status: failed` — a failure, but for the wrong thing. It
reports that the *policy* is malformed, not that the *requirement* is unmet, so an adopter who tries
it converts a silent gap into a misleading finding about their configuration. That is a worse
outcome than the silence, and it is why the route counts as refused rather than merely awkward.
...
- **Does a rejection expire?** An approval goes stale as the code moves. A rejection describes a gap
  ...
  **This question has since been answered empirically, and the answer is that the asymmetry already
  exists and was never decided.** `judgeAttestation` returns the rejection at
  `scripts/compliance.mjs:212`, *before* the expiry and digest checks below it. A rejected
  attestation is therefore exempt from freshness entirely, and its recorded `reviewedAgainst.digest`
  is consulted by nothing. Verified against the implementation during reconciliation:

  ```text
  approved + reviewed content changes  → not-evaluated / skipped   (stale, returns to unestablished)
  rejected + reviewed content changes  → attested-rejected / failed (unchanged, permanently)
  ```

  The live instance is `errors.no-false-success` in this repository's own policy: rejected at 2.0.0,
  the code it was rejected over subsequently rewritten, and the rejection undisturbed. That is not
  obviously wrong — a violation should not evaporate because a file moved — but it was not chosen,
  and it means the model cannot distinguish *the conduct was found unmet* from *the implementation
  has since changed enough to need another look*. Whichever way this ADR resolves, it now has to
  resolve that too.
...
**Stated as a proposed direction, not as a property this repository has.** It is worth being exact
here, because the sentence reads like a description and is not one: the framework currently
*violates* it, demonstrably and in this very area. A rule that was reviewed and reached no verdict
renders in the same bucket as one nobody looked at — `architecture.no-boundary-bypass` spent a review
cycle filed under *"Unestablished prohibitions — nobody looked for these"* while somebody had. The
classification was right and the sentence was wrong, and there is no third state to move it to. That
counterevidence is what makes the principle worth pursuing rather than what disqualifies it, but it
has to be recorded as a direction the design should move in. Promoting it into a normative standard
is a separate decision needing its own evidence, and it is deliberately not made here or in
[Standard 45](../../standards/45-engineering-invariants.md).
