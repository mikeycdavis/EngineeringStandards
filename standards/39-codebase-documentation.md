...
Partially met, and it contains **the clearest standards conflict found so far**.

**Met.** `docs/architecture.md` exists, was generated by `/codebase-docs`, and describes
responsibilities and interactions rather than listing directories — R1's trigger and R3's test are
both satisfied. `README.md` documents artifact, plan, and ADR locations, covering four of R5's
twenty onboarding items.

**R4 is violated, and the violating instruction is in the skill.** `docs/architecture.svg` is a
hand-authored SVG, produced by `/codebase-docs` from `references/svg-guide.md`, which specifies
hand-written SVG with computed coordinates. No `.mmd` source exists. This standard requires Mermaid
as canonical source and prohibits maintaining a hand-edited SVG where the SVG represents a diagram —
so the repository is non-compliant *and* the tool that produced it will reproduce the violation on
every run.

This is not a defect to fix in the repository alone. Per [Standard 34](34-dogfooding.md) R5, the fix
belongs where the defect is: the `/codebase-docs` skill needs to emit `.mmd` source and derive the
SVG from it. Until then, regenerating the documentation restores the violation, which makes this the
highest-value item surfaced by writing this standard.
