...
| --- | --- |
| R1 — generates the artifacts | `project-policy.yml`, `PROJECT.md`, `AGENTS.md`, `CLAUDE.md`, `.github/copilot-instructions.md`, `artifacts/project-plan-breakdown/`, `artifacts/adr/`, from `templates/`. All seven the source names ([Standard 17](17-agent-instruction-files.md)). A test asserts what it writes validates against the real schema, and another that every template it names exists |
